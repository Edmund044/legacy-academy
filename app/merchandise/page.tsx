...
export default function MerchandisePage() {
  ...
  const handleMerchandise = async (equipmentId:string,stock_total:number) => {
    try {
      await apiClient({
        endpoint: `/v1/equipment/inventory/${equipmentId}`,
        method: "PATCH",
        headers: {
  ...
  return (
    <>
      <PageHeader title="Kits & Merchandise" description="Buy academy kits, equipment, and merchandise.">
      </PageHeader>
{/* 
      ...
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <div className="flex gap-1">
            {["All Equipment", "balls", "training_gear", "field_equipment", "medical_kits"].map(c => (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {filtered.map((p, i) => (
              <div key={i} className="rounded-xl border border-border overflow-hidden hover:shadow-md transition-shadow group">
                <div className={`h-32 bg-green-600 relative flex items-center justify-center`}>
                  <ShoppingBag className="w-10 h-10 text-white/60" />
                  {p.category && <span className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-0.5 rounded ${p.name === "BEST SELLER" ? "bg-green-500 text-white" : "bg-amber-500 text-white"}`}>{p.category}</span>}
                ...
                <div className="p-3">
                  ...
                  <p className="text-sm font-bold text-brand mt-1">KES {p.replacement_cost_usd}</p>
            ...
            onSuccess={() => handleMerchandise(p.id,p.stock_total)}
